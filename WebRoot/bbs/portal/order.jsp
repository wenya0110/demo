...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %> <!-- 引用jstl标签库里常用函数 -->
<%
...
<!--结算页面样式-->
<link rel="stylesheet" type="text/css" href="<%=basePath  %>bbs/css/base.css" media="all" />
<script type="text/javascript" src="<%=basePath  %>bbs/js/jquery-1.6.4.js"></script>
<script type="text/javascript" src="<%=basePath  %>bbs/js/base.js"></script>
...
<body id="mainframe">
	...
	<div class="w w1 header clearfix">
		<div id="logo">
			<a href="/"><img src="<%=basePath  %>bbs/images/XBB2.png" alt="新巴巴商城"></a> <a
	...
	<!-- main -->
<form action="<%=basePath %>order/setorder" method="post"> <!-- action为安全属性   post对数据加密并且长度不限 -->
	<div id="container">
												...
												<ul id="consignee-list">
													<li
														...
														<div class="consignee-item item-selected"
															...
															<span limit="8" title="范冰冰">${data[0].real_name } ${data[0].province }</span><b></b>
														</div>
														<div class="addr-detail">
															<span title="范冰冰 " class="addr-name" limit="6">${data[0].real_name }
															</span> <span class="addr-info" limit="45">${data[0].province } ${data[0].city } ${data[0].town }
																${data[0].addr }</span> <span class="addr-tel">${data[0].email }</span>
														</div>
						...
						<div id="shipAndSkuInfo">
							<div id="payShipAndSkuInfo">
								...
								<div class="step-tit">
									<h3>送货清单</h3>
									<div class="extra-r">
										<a href="<%=basePath %>cart/getcarts" class="return-edit ftx-05">返回修改购物车</a>
									</div>
								</div>
										...
										<div class="shopping-list ABTest">
											<div class="goods-list">
												...
												<div class="goods-items">
													...
													<c:forEach items="${data }" begin="1" var="p" varStatus="s">
													<input type="hidden" name="list[${s.index-1}].productId" value="${p.productid }"/>
													<input type="hidden" name="list[${s.index-1}].productName" value="${p.productname }"/>
													<input type="hidden" name="list[${s.index-1}].size" value="${p.sizename }"/>
													<input type="hidden" name="list[${s.index-1}].color" value="${p.colorname }"/>
													<input type="hidden" name="list[${s.index-1}].price" value="${p.price }"/>
													<input type="hidden" name="list[${s.index-1}].amount" value="${p.num }"/>
														...
														<div class="goods-item goods-item-extra"
															...
															<div class="p-img">
																<a target="_blank"
																	...
																	src="<%=basePath %>bbs${p.imgs[0] }" alt=""></a>
															</div>
															<div class="goods-msg">
																<div class="goods-msg-gel">
																	<div class="">
																		<a href="http://item.jd.com/1305606.html"
																			target="_blank">${p.productname } -- ${p.colorname } -- ${p.sizename }</a>
																	</div>
																	<div class="p-price">
			...
			<div class="order-summary">
				...
				<div class="statistic fr">
					<div class="list">
						...
						<c:forEach items="${data }" var="p" begin="1" >
							<c:set var="price" value="${price+p.price*p.num }" ></c:set>
							<c:set var="fee" value="${fee+10 }" ></c:set>
						</c:forEach>
						<span><em class="ftx-01"><c:out value="${ fn:length(data)-1 }"></c:out></em> 件商品，总商品金额：</span> <em
							class="price" id="warePriceId" v="343.8">￥<c:out value="${price }"></c:out></em>
					</div>
					<div class="list">
					...
					<div class="list">
						<span><i class="freight-icon"></i>运费：</span> <em class="price"
							id="freightPriceId"><font color="#FF6600"> ￥<c:out value="${fee }"></c:out></font></em>
					</div>
					...
					<div class="list">
						<span>应付总额：</span> <em class="price" id="sumPayPriceId">￥<c:out value="${fee+price }"></c:out></em>
					</div>
				</div>
				<div class="clr"></div>
			</div>
						...
						<div class="sticky-wrap">
							<div class="inner">
								<input type="submit" class="checkout-submit" value="提交订单" /> <span
									class="total">应付总额：<strong id="payPrice">￥<c:out value="${fee+price }"></c:out></strong>
							</div>
						</div>
					</div>
				</div>
	...
	<input name="deliverFee" value="${fee }" type="hidden"/>
	<input name="buyerId" value="1" type="hidden"/>
	<input name="isConfirm" value="1" type="hidden"/>
	<input name="isPaiy" value="2" type="hidden"/>
	<input name="orderState" value="0" type="hidden"/>
	<input name="totalPrice" value="${fee+price }" type="hidden"/>
	<input name="orderPrice" value="${price }" type="hidden"/>
	<input name="ids" value="${skuids }" type="hidden"/>
	<input name="nums" value="${nums }" type="hidden"/>
</form>
	<!-- /main -->
